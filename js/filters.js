...
function updateRecipes() {
    ...
    if (mainSearchValue != "") {
        //remove recipe from list if keyword not found in name/description/ingredients
        matches = matches.filter(match => strFound(mainSearchValue, [match.name, match.ingredients, match.description]))
    }
    
    categories.forEach((category) => {
        category.actives.forEach((active) => {
            //remove recipe from list if filter not found in given recipe property (ingredients/ustensils/appliance)
            matches = matches.filter(match => strFound(active, match[category.name]))
        })
    })
